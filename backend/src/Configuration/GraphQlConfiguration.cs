            ...
            )
        {
            ...
            .AddHttpRequestInterceptor(async (httpContext, requestExecutor, requestBuilder, cancellationToken) =>
            {
                try {
                    ...
                    // grasp the various authentication flows. For the Web frontend,
                    // the database acts as OpenId Connect Client of the metabase
                    // and uses the cookie scheme for authentication. For
                    // third-party frontends, the database acts as resource server
                    // and uses authorization header bearer tokens for
                    // authentication, that is JavaScript Web Tokens (JWT), aka,
                    // Access Tokens, provided as `Authorization` HTTP header with
                    // the prefix `Bearer` as issued by OpenIddict. This Access
                    // Token includes Scopes and Claims.
